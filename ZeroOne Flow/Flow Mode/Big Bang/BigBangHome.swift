...
struct BigBangHome: View {
    ...
    var body: some View {
        ...
        Section {
            SandwichedImageSlider(interval: $bigbang.interval, min: 0.01, max: 0.2)
        } header: {
            Text("Speed")
        }
        
        Section {
            CGFloatSlider(value: $bigbang.gap, min: 0.01, max: 200, image: "arrow.up.left.and.arrow.down.right",
        ...
        Section {
            SliderWithSingleImage(value: $bigbang.rotationAngle, min: 0, max: 360, image: "crop.rotate", format: "%.0f")
            ToggleWithLabel(value: $bigbang.is3D, label: "3D", trueImage: "move.3d", falseImage: "move.3d")
        } header: {
            Text("Rotation")
        }
