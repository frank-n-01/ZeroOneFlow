...
struct RainFlow: View {
    ...
    var body: some View {
        ...
        .onReceive(Timer.publish(every: rain.start ? rain.interval : 100, on: .current, in: .common).autoconnect()) { _ in
            NotificationCenter.default.post(name: Notification.Name("rain"), object: nil)
            if count < loop {
                count += 1
            }
        }
        ...
        Text(content)
            ...
            .onAppear {
                ...
                for _ in 0 ..< Int.random(in: 1...Int(rain.length)) {
                    content += ContentMaker.makeContent(with: rain.contents) + "\n"
                }
                position.x = CGFloat.random(in: 0...width)
                position.y = CGFloat.random(in: 0...height)
            }
            .onReceive(NotificationCenter.default.publisher(for: Notification.Name("rain"))) { _ in
                move()
            }
    }
