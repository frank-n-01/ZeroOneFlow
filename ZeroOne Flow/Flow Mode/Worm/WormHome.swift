...
struct WormHome: View {
    ...
    var body: some View {
        ...
        Section {
            SandwichedImageSlider(interval: $worm.interval, min: 0.01, max: 1.0)
        } header: {
            Text("Speed")
        }
        
        Section {
            SliderWithSingleImage(value: $worm.step, min: 1, max: 300, image: "arrowshape.zigzag.right.fill", format: "%.0f")
        } header: {
            Text("Step")
        }

        Section {
            SliderWithSingleImage(value: $worm.crawling, min: 0, max: 200, image: "scribble.variable", format: "%.0f")
        } header: {
            Text("Crawling")
        }
